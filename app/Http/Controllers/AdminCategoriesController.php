<?php namespace App\Http\Controllers;
	...
	class AdminCategoriesController extends \crocodicstudio_voila\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
	        ...
	        $this->col = [];
			$this->col[] = array("label"=>"Active","name"=>"active" );
		$this->col[] = array("label"=>"Sorting","name"=>"sorting" );
		$this->col[] = array("label"=>"Name En","name"=>"name_en" );
		$this->col[] = array("label"=>"Name Fr","name"=>"name_fr" );
		$this->col[] = array("label"=>"Breif En","name"=>"breif_en" );
		$this->col[] = array("label"=>"Breif Fr","name"=>"breif_fr" );
		$this->col[] = array("label"=>"Image","name"=>"image" ,"image"=>true);
			...
			$this->form[] = ['label'=>'Image','name'=>'image','type'=>'filemanager','width'=>'col-sm-10','help'=>'340 * 250'];
			$this->form[] = ['label'=>'Parent','name'=>'parent_id','type'=>'select2','width'=>'col-sm-10','datatable'=>'categories,name_en'];
			...
			//$this->form[] = ['label'=>'Image','name'=>'image','type'=>'filemanager','width'=>'col-sm-10','help'=>'370 * 242'];
			//$this->form[] = ['label'=>'Parent','name'=>'parent_id','type'=>'select2','width'=>'col-sm-10','datatable'=>'categories,name_en'];
			//$this->form[] = ['label'=>'Active','name'=>'active','type'=>'radio','width'=>'col-sm-10','dataenum'=>'1|yes;0|no'];
			# OLD END FORM
    ...
    | Hook for execute command after delete public static function called
	    ...
	    */
		...
		public function getIndex() {
			...
			$allCategories = Category::orderBy('sorting','asc')->get();
		...
		public function getToSort($parent_id)
		{
			$categories = Category::where('parent_id', '=', $parent_id)->orderBy('sorting','asc')->get();

			$arr=Array("data"=>$categories);

			return response()->json($arr,200);
		}


		public function ordring(Request $request)
		{
				$Categories = Category::where('parent_id',$request->input('parent_id'))->get();

				foreach ($Categories as $Category) {
					foreach ($request->input('order') as $order) {
						if ($order['id'] == $Category->id) {
							$Category->update(['sorting' => $order['position']]);
						}
					}
				}
