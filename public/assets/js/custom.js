...
(function ($) {
    ...
    $(document).on('ready', function () {
     ...
     function windowSizeCheck() {
            $('.navbar-nav li').off('click.mobile'); // Prevent duplicate bindings
            if($(window).width() < '768') {
                $('.navbar-nav li').on('click.mobile', function(e) {
                console.log(e);
                    var childMenu = e.currentTarget.children[1];
                    var childhref = e.currentTarget.children[0];
                    if($(childMenu).hasClass('open')) {
                        $(childMenu).removeClass('open');
                    }else{
                        $(childMenu).addClass('open');
                        $(childMenu).siblings().removeAttr('href');
                    }
                });
             }
        }
...
jQuery('.mob-search').on('click', function(event) {        
        jQuery('.header_area').toggle('show');
    });